...
The [4x4 board solutions](#fig-soluciones-del-tablero-4x4) present both solutions for the eight queens problem on a 4x4 board. The first solution consists of placing the queens on squares a2, b4, c1 and d3. In the second solution, the queens move to the squares indicated by the arrows, being positioned on squares a3, c4, b1 and d2.
...
An initial idea commonly considered is to represent the position of a queen through its coordinates on the board. Those queens that have not yet been placed are simply not included in the list of coordinates. The [board used for the representation](#fig-tablero-para-mostrar-la-representacion) shows the board that will be used to show the different representations.
...
Therefore, the new representation of the board in the [representation board](#fig-tablero-para-mostrar-la-representacion) would be:
...
Two queens, represented by positions $i$ and $j$ in a vector $V$, are on the same diagonal if and only if $\lvert i - j\rvert = \lvert V[i] - V[j]\rvert$. In other words, the absolute difference between columns $i$ and $j$ must be equal to the absolute difference between rows $V[i]$ and $V[j]$. The absolute value operator, $\lvert\cdot\rvert$, is used to eliminate any negative sign. For example, $\lvert2\rvert = 2$ and $\lvert-3\rvert = 3$.
...
Once an example has been analyzed and its operation understood, we can define the algorithm through a flowchart that illustrates the general resolution process. The [N-queens backtracking flowchart](#fig-diagrama-de-flujo-de-backtracking-aplicado-a-n-damas) shows said flowchart. In a real program, it would be necessary to detail more specifically the actions performed in each step of the flowchart, but since this book has an informative character, it is not necessary to go into those technical details.
